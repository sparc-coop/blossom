...
public class SparcEngineAuthenticator<T>(
    IRepository<T> users,
    ISparcEngine engine,
...
{
    public Lazy<Task<IJSObjectReference>> Js { get; } = new(() => js.InvokeAsync<IJSObjectReference>("import", "./_content/Sparc.Blossom/SparcEngineAuthenticator.js").AsTask());

    private ITimer? Timer;

    const string PublicKey = "sparcengine:public:63cc565eb9544940ad6f2c387b228677";

    public override async Task<BlossomUser> LoginAsync(ClaimsPrincipal principal, string? emailOrToken = null)
    {
        User = await engine.UserInfo();
