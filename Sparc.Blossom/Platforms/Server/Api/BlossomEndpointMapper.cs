...
public class BlossomEndpointMapper(Assembly assembly)
{
    ...
    public static void MapEndpoints<T>(IEndpointRouteBuilder endpoints)
    {
        ...
        group.MapPost("_queries/{name}", async (IRunner<T> runner, string name, object[] parameters) => await runner.ExecuteQuery(name, parameters));
