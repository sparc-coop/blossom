...
public partial class BlossomAuthenticator : AuthenticationStateProvider, IAccessTokenProvider
{
    ...
    static async Task<WebAuthenticatorResult> PlatformAuthenticateAsync(WebAuthenticatorOptions webAuthenticatorOptions)
    {
        ...
        var parentActivity = Platform.CurrentActivity;// Platform.GetCurrentActivity(true);
        ...
        try
        {
            if (await BindServiceAsync(customTabsActivityManager))
            {
                ...
                BlossomAuthenticatorIntermediateActivity.StartActivity(parentActivity, customTabsIntent.Intent);
            }
            else
            {
