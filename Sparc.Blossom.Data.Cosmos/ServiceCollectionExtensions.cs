...
public static class ServiceCollectionExtensions
{
    public static IServiceCollection AddCosmos<T>(this IServiceCollection services, string connectionString, string databaseName, ServiceLifetime serviceLifetime = ServiceLifetime.Scoped) where T : DbContext
    {
        ...
        services.AddSingleton(sp => CosmosDbSimpleClient<T>.CreateClient(sp.GetRequiredService<IConfiguration>()));
        services.Add(new ServiceDescriptor(typeof(CosmosDbSimpleClient<>), typeof(CosmosDbSimpleClient<>), serviceLifetime));
